...
// ***************************************************************** //
//
// An efficient assembly routine that builds the linear and nonlinear parts separately
// nonlinear parts include bits that could be attributed to 
...
void
Picard::assemble_efficient (ErrorVector &)	// error_vector)
{
  ...
  unsigned int count = 0;
	...
	for (; el != end_el; ++el)
	{
		...
		if (std::find (subdomains_3d.begin (), subdomains_3d.end (),elem->subdomain_id ()) != subdomains_3d.end ())
	  	{
			...
			for (unsigned int qp = 0; qp < qrule.n_points (); qp++)
				...
				*/
						...
						if(assemble_pressure_convection_diffusion_matrix)
						{
						    	...
						    	if (streamline_diffusion && !stokes)
							{
								if(newton < 3) // implicit
								  ...
								  Kpp_pre_convection_diffusion (i, j) += sd_param * JxW[qp] * (U_old * dpsi[i][qp]) * (U_old * dpsi[j][qp]);

							}
						}
					}	// end j pressure dof loop
				}	// end i pressure dof loop

				if(assemble_velocity_mass_matrix)
				{
					for (unsigned int i = 0; i < n_u_dofs; i++)
					{
						for (unsigned int j = 0; j < n_u_dofs; j++)
				...
				if(assemble_velocity_matrix)
				{
					for (unsigned int i = 0; i < n_u_dofs; i++)
					{
						for (unsigned int j = 0; j < n_u_dofs; j++)
						{
							...
							if (unsteady)
							{
								...
								if (threed)
								{
									Kww_pre_velocity (i, j) += JxW[qp] / dt * (phi[i][qp] * phi[j][qp]);
								}	// mass matrix term
							}
						...
						for (unsigned int j = 0; j < n_p_dofs; j++)
						{
							...
							if (threed)
							  Kwp_pre_velocity (i, j) += -JxW[qp] * psi[j][qp] * dphi[i][qp] (2);
						}	// end j pressure dof loop
					} // end i velocity dof loop
					...
					for (unsigned int i = 0; i < n_p_dofs; i++)
					{
						...
						for (unsigned int j = 0; j < n_u_dofs; j++)
						{
							...
							if (threed)
							{
								Kpw_pre_velocity (i, j) += -JxW[qp] * psi[i][qp] * dphi[j][qp] (2);
							}
						}
				...
				} // end preconditioner velocity matrix assemble
			  	...
			  	if (supg || pspg || lsic)
				{
					...
					if (unsteady && supg_constant_constant)
					{
					  velocity = U_old;
					  grad_V_u = grad_u_old;
					  grad_V_v = grad_v_old;
					...
					if (threed)
					{
					  grad_Vx =	NumberVectorValue (grad_V_u (0), grad_V_v (0), grad_V_w (0));
					  grad_Vy =	NumberVectorValue (grad_V_u (1), grad_V_v (1), grad_V_w (1));
					  grad_Vz =	NumberVectorValue (grad_V_u (2), grad_V_v (2), grad_V_w (2));
					  r =	NumberVectorValue (grad_Vx * velocity, grad_Vy * velocity, grad_Vz * velocity);
					  r = r.unit ();
					}
					else
					{
					  grad_Vx =	NumberVectorValue (grad_V_u (0), grad_V_v (0));
					  grad_Vy =	NumberVectorValue (grad_V_u (1), grad_V_v (1));
					  r =	NumberVectorValue (grad_Vx * velocity, grad_Vy * velocity);
					  r = r.unit ();
					}
					...
					else
					{
					  if (supg_parameter == 0)	//not great nonlinear conv but better
						...
						else if (supg_parameter == 1)	// slow convergence (none with changing)
						{
							//static characteristic element length
							//double h_p = pow(6*elem_volume/pi,1.0/3.0)/sqrt(3.0);
							double grad_times_velocity = 0.;
							...
							for (unsigned int i = 0; i < n_u_dofs; i++)
							{
								grad_times_velocity += fabs (dphi[i][qp] * velocity);
							}
							double tau_sugn12 = 1. / grad_times_velocity;

							double tau_sugn3 = h_rgn * h_rgn * reynolds_supg / 4.;

							tau_supg = 1. / sqrt (1. / (tau_sugn12 * tau_sugn12) + 1. / (tau_sugn3 * tau_sugn3));
							tau_pspg = tau_supg;
							tau_lsic = tau_supg * velocity_mag * velocity_mag;

						}
						else if (supg_parameter == 2)	//good convecgence
						{
							double grad_times_velocity = 0.;
							//double grad_times_r = 0.;	// unused
							...
							double h_ugn = 2. * velocity_mag / grad_times_velocity;
							double tau_sugn1 = h_ugn / 2. / velocity_mag;
							double tau_sugn2 = dt / 2.;
							double tau_sugn3 = h_ugn * h_ugn * reynolds_supg / 4.;

							double Re_ugn =	velocity_mag * h_ugn * reynolds_supg / 2.;
							double z = 0.;
							if (Re_ugn <= 3.)
								z = Re_ugn / 3.;
							else
							...
							tau_supg = 1. / sqrt (1. / (tau_sugn1 * tau_sugn1) + 1. / (tau_sugn2 * tau_sugn2) + 1. / (tau_sugn3 * tau_sugn3));
							tau_pspg = tau_supg;
					...
					// now that we have set up the parameter, we can use s different velocity 
					// for the picard/newton iterations if we want. 
					// if using semi-implicit, newton - 3, then we need to use the velocities 
					// and grads from the previous timestep.
					// not we can't use ismail parameters when steady sim
					if (newton == 3) //false)	//unsteady)
					{
						velocity = U_old;
						grad_V_u = grad_u_old;
						grad_V_v = grad_v_old;
						  	...
						  	if (unsteady)
								{
									...
									if (threed)
									{
										Fw (i) -=	-tau_supg * JxW[qp] / dt * (velocity * dphi[i][qp] * w_old);
									}
								}
								...
								if (!stokes)
								{
									...
									if (threed)
									{
										Fw (i) +=	tau_supg * JxW[qp] * ((velocity * dphi[i][qp]) * (U * grad_w));
									}
								}
								...
								for (unsigned int j = 0; j < n_u_dofs; j++)
								{
									...
									if (threed)
									{
										laplacian_operator +=	d2phi[j][qp] (2, 2);
									}
									...
									if (!stokes)
									{
										...
										if (threed)
										{
											Kuw (i, j) += tau_supg * JxW[qp] * (phi[j][qp] * grad_u (2) *  velocity *  dphi[i][qp]);
										}
										...
										if (threed)
										{
											Kvw (i, j) +=	tau_supg * JxW[qp] * (phi[j][qp] *grad_v (2) *velocity *dphi[i][qp]);
										}
										if (threed)
										{
											Kwu (i, j) += tau_supg * JxW[qp] * (phi[j][qp] * grad_w (0) * velocity * dphi[i][qp]);
											Kwv (i, j) +=	tau_supg * JxW[qp] * (phi[j][qp] *grad_w (1) * velocity *	dphi[i][qp]);
											Kww (i, j) += tau_supg * JxW[qp] * (phi[j][qp] * grad_w (2) * velocity * dphi[i][qp] + (U * dphi[j][qp]) * (velocity * dphi[i][qp]));
										}
									}
									...
									if (unsteady)
									{
										...
										if (threed)
										{
											Kww (i, j) += tau_supg * JxW[qp] * (1.0 / dt * velocity * dphi[i][qp] * phi[j][qp]);
										}
									}
								}
								...
								for (unsigned int j = 0; j < n_p_dofs; j++)
								{
									...
									if (threed)
									{
										Kwp (i, j) += tau_supg * JxW[qp] * (velocity * dphi[i][qp] * dpsi[j][qp](2));
									}
								}
							}
						}
						else if (supg_full_newton)
						{
							...
							for (unsigned int i = 0; i < n_u_dofs; i++)
							{
								...
								if (unsteady)
								{
									...
									if (threed)
									{
										Fw (i) -=	-tau_supg * JxW[qp] / dt * (U *	dphi[i][qp] * w);
									}
								}
								...
								if (!stokes)
								{
									...
									if (threed)
									{
										Fw (i) +=	2 * tau_supg * JxW[qp] *((U * dphi[i][qp]) * (U * grad_w));
									}
								}
								...
								for (unsigned int j = 0; j < n_u_dofs; j++)
								{
									...
									if (!stokes)
									{
										...
										if (threed)
										{
											Kuw (i, j) += tau_supg * JxW[qp] * (U * grad_u * phi[j][qp] *	dphi[i][qp](2) + U * dphi[i][qp]	*	phi[j][qp] * grad_u (2));
										...
										if (threed)
										{
											Kvw (i, j) +=	tau_supg * JxW[qp] * (U * grad_v * phi[j][qp] * dphi[i][qp](2) +	U *	dphi[i][qp]	*	phi[j][qp] * grad_v (2));
										}	// + dphi[i][qp](2) * phi[j][qp]*grad_p(1)); }     
					  
										if (threed)
										{
											Kwu (i, j) +=	tau_supg * JxW[qp] * (U * grad_w * phi[j][qp] *	dphi[i][qp](0) + U *	dphi[i][qp]	*	phi[j][qp] * grad_w (0));	// + dphi[i][qp](0) * phi[j][qp]*grad_p(2)); }             
											Kwv (i, j) +=	tau_supg * JxW[qp] * (U * grad_w * phi[j][qp] *	dphi[i][qp](1) + U *	dphi[i][qp]	*	phi[j][qp] * grad_w (1)); // + dphi[i][qp](1) * phi[j][qp]*grad_p(2)); }
											Kww (i, j) +=	tau_supg * JxW[qp] * (U * grad_w * phi[j][qp] *	dphi[i][qp](2) + (U * dphi[i][qp]) * (U * dphi[j][qp]) + U * dphi[i][qp] * phi[j][qp] *	grad_w (2));
									...
									if (unsteady)
									{
										...
										if (threed)
										{
											Kww (i, j) += tau_supg * JxW[qp] / dt * (U * dphi[i][qp] * phi[j][qp]);
										}
										...
										if (threed)
										{
											Kuw (i, j) +=	tau_supg * JxW[qp] / dt * (u * dphi[i][qp](2) * phi[j][qp] - u_old * dphi[i][qp](2) * phi[j][qp]);
										}
										...
										{
											Kvw (i, j) +=
											tau_supg * JxW[qp] / dt * (v * dphi[i][qp](2) * phi[j][qp] - v_old * dphi[i][qp](2) * phi[j][qp]);
										}

										if (threed)
										{
											Kwu (i, j) += tau_supg * JxW[qp] / dt * (w * dphi[i][qp](0) * phi[j][qp] - w_old * dphi[i][qp](0) * phi[j][qp]);
											Kwv (i, j) += tau_supg * JxW[qp] / dt * (w * dphi[i][qp](1) * phi[j][qp] - w_old * dphi[i][qp](1) * phi[j][qp]);
											Kww (i, j) += tau_supg * JxW[qp] / dt * (w * dphi[i][qp](2) * phi[j][qp] - w_old * dphi[i][qp](2) * phi[j][qp]);
										}
								...
								for (unsigned int j = 0; j < n_p_dofs; j++)
								{
									...
									if (threed)
									{
										Kwp (i, j) += tau_supg * JxW[qp] * (U * dphi[i][qp] * dpsi[j][qp](2));
									}
								}
							}

						}
						else if (supg_convection_newton)
						{
							...
							for (unsigned int i = 0; i < n_u_dofs; i++)
							{
								...
								for (unsigned int j = 0; j < n_u_dofs; j++)
								{
									...
									if (!stokes)
									{
										Kuu (i, j) +=	tau_supg * JxW[qp] * (U * grad_u * phi[j][qp] * dphi[i][qp] (0) + (U * dphi[i][qp]) * (U * dphi[j][qp]) + U * dphi[i][qp] * phi[j][qp] * grad_u (0));
										Kuv (i, j) +=	tau_supg * JxW[qp] * (U * grad_u * phi[j][qp] * dphi[i][qp] (1) + U * dphi[i][qp] * phi[j][qp] * grad_u (1));
										if (threed)
										{
											Kuw (i, j) += tau_supg * JxW[qp] * (U * grad_u * phi[j][qp] * dphi[i][qp](2) + U * dphi[i][qp] * phi[j][qp] *	grad_u (2));
										}
								
										Kvu (i, j) += tau_supg * JxW[qp] * (U * grad_v * phi[j][qp] * dphi[i][qp] (0) + U * dphi[i][qp] * phi[j][qp] * grad_v (0));
										Kvv (i, j) +=	tau_supg * JxW[qp] * (U * grad_v * phi[j][qp] * dphi[i][qp] (1) + (U * dphi[i][qp]) * (U * dphi[j][qp]) + U * dphi[i][qp] * phi[j][qp] * grad_v (1));
										if (threed)
										{
											Kvw (i, j) += tau_supg * JxW[qp] * (U * grad_v * phi[j][qp] * dphi[i][qp](2) + U * dphi[i][qp] * phi[j][qp] * grad_v (2));
										}
										...
										if (threed)
										{
											Kwu (i, j) += tau_supg * JxW[qp] * (U * grad_w * phi[j][qp] * dphi[i][qp](0) + U * dphi[i][qp] * phi[j][qp] * grad_w (0));
											Kwv (i, j) += tau_supg * JxW[qp] * (U * grad_w * phi[j][qp] * dphi[i][qp](1) + U * dphi[i][qp] * phi[j][qp] * grad_w (1));
											Kww (i, j) += tau_supg * JxW[qp] * (U * grad_w * phi[j][qp] * dphi[i][qp](2) + (U * dphi[i][qp]) * (U * dphi[j][qp]) + U * dphi[i][qp] * phi[j][qp] * grad_w (2));
										}
									}
									if (supg_laplacian)
						...
						else if (supg_picard)
						{
							...
							for (unsigned int i = 0; i < n_u_dofs; i++)
							{
								...
								for (unsigned int j = 0; j < n_u_dofs; j++)
								{
									...
									if (!stokes)
									{
										Kuu (i, j) +=	tau_supg * JxW[qp] * ((velocity * dphi[j][qp]) * (velocity * dphi[i][qp]));
										Kvv (i, j) += tau_supg * JxW[qp] * ((velocity * dphi[j][qp]) * (velocity * dphi[i][qp]));
										if (threed)
										{
											Kww (i, j) += tau_supg * JxW[qp] * ((velocity * dphi[j][qp]) * (velocity * dphi[i][qp]));
										}
									}
									if (supg_laplacian)
									{
					...
					if (lsic)
					{
						for (unsigned int i = 0; i < n_u_dofs; i++)
						{
							for (unsigned int j = 0; j < n_u_dofs; j++)
							{
								...
								if (threed)
								{
									Kuw (i, j) += tau_lsic * JxW[qp] * (dphi[i][qp] (0) * dphi[j][qp] (2));
								}
								...
								if (threed)
								{
									Kvw (i, j) += tau_lsic * JxW[qp] * (dphi[i][qp] (1) * dphi[j][qp] (2));
								}
			
								if (threed)
								{
									Kwu (i, j) += tau_lsic * JxW[qp] * (dphi[i][qp] (2) * dphi[j][qp] (0));
									Kwv (i, j) += tau_lsic * JxW[qp] * (dphi[i][qp] (2) * dphi[j][qp] (1));
									Kww (i, j) += tau_lsic * JxW[qp] * (dphi[i][qp] (2) * dphi[j][qp] (2));
								}
							}
						}
					}
					...
					if (pspg)
					{
						...
						if (pspg_newton)
						{
							...
							if (threed)
							{
								grad_Ux =	NumberVectorValue (grad_u (0), grad_v (0), grad_w (0));
								grad_Uy = NumberVectorValue (grad_u (1), grad_v (1), grad_w (1));
								grad_Uz = NumberVectorValue (grad_u (2), grad_v (2), grad_w (2));
							}
							else
							{
								grad_Ux = NumberVectorValue (grad_u (0), grad_v (0));
								grad_Uy = NumberVectorValue (grad_u (1), grad_v (1));
							}
							...
							for (unsigned int i = 0; i < n_p_dofs; i++)
							{
								...
								if (!stokes)
								{
									...
									if (threed)
									{
										Fp (i) += -tau_pspg * JxW[qp] * (dpsi[i][qp] (2) * (U * grad_w));
									}
								}
								...
								for (unsigned int j = 0; j < n_u_dofs; j++)
								{
									...
									if (!stokes)
									{
										...
										if (threed)
										{
											Kpw (i, j) += -tau_pspg * JxW[qp] * (phi[j][qp] * grad_Uz * dpsi[i][qp] + (U * dphi[j][qp]) * dpsi[i][qp] (2));
										}
									}
									...
									if (unsteady)
									{
										...
										if (threed)
										{
											Kpw (i, j) += -tau_pspg * JxW[qp] * (1.0 / dt * dpsi[i][qp] (2) * phi[j][qp]);
										}
									}
								}
						...
						else if (pspg_picard)
						{
							...
							for (unsigned int i = 0; i < n_p_dofs; i++)
							{
								...
								for (unsigned int j = 0; j < n_u_dofs; j++)
								{
									...
									if (!stokes)
									{
										Kpu (i, j) += -tau_pspg * JxW[qp] *	((velocity * dphi[j][qp]) * dpsi[i][qp] (0));
										Kpv (i, j) +=	-tau_pspg * JxW[qp] *	((velocity * dphi[j][qp]) * dpsi[i][qp] (1));
										if (threed)
										{
											Kpw (i, j) += -tau_pspg * JxW[qp] * ((velocity * dphi[j][qp]) * dpsi[i][qp] (2));
										}
									}
									if (supg_laplacian)
									{
			...
			// **************** BOUNDARY TERMS/CONDITIONS **************//
			{
				...
				for (unsigned int s = 0; s < elem->n_sides (); s++)
				{
					...
					if (boundary_ids.size () > 0)
					{
						...
						if ((preconditioner_type_3d == 4 || preconditioner_type_3d == 5) && problem_type != 4)
					  	{
							if (boundary_id == 0)
							{

								if (pcd_boundary_condition_type	== 1
										|| pcd_boundary_condition_type == 3
										|| pcd_boundary_condition_type == 4)
						  	{
									...
									for (unsigned int qp = 0; qp < qface.n_points (); qp++)
									{
										...
										for (unsigned int l = 0; l < n_u_dofs; l++)
										{
											...
											else // semi-implicit
											{
												...
												u_old += phi_face[l][qp] * system->old_solution (dof_indices_u[l]);
												v_old += phi_face[l][qp] * system->old_solution (dof_indices_v[l]);
												if (threed)
		...
		*/
			...
			if (!estimating_error)
			{
				...
				if(assemble_pressure_mass_matrix || assemble_scaled_pressure_mass_matrix)
				...
				*/
				...
				if ((preconditioner_type_3d == 4 || preconditioner_type_3d == 5) && problem_type != 4)
				{
					if (pcd_boundary_condition_type == 1 || pcd_boundary_condition_type == 3
				  		|| pcd_boundary_condition_type == 4)
					{
	...
	if (pcd_boundary_condition_type == 3 && (assemble_pressure_laplacian_matrix || assemble_pressure_convection_diffusion_matrix))
 	{
		// sort the dofs
