...
% in the inexact preconditioner:
% three preconditioners:        diagonal, schur_0d, schur stokes and
% partitioned
% two types bcs:                asymmetric or symmetric geometries
% three numbers of outlets:     2, 8 and 32 outlets
% three timestep sizes:         0.01,0.001


preconditioner_type = cellstr(['diagonal/           '; 'schur_0d/           '; 'schur_stokes/       '; 'schur_stokes_gmres/ '; 'schur_stokes_rerun/ '; 'schur_stokes_switch/';  'partitioned/        '; 'uncoupled/          ']);
bc_type = cellstr(['asym/'; 'sym/ ']);
num_outlets = cellstr(['outlets_2/ '; 'outlets_8/ '; 'outlets_32/']);
time_steps = cellstr(['dt_0.01/ '; 'dt_0.001/']);
time_steps_double = [0.01;0.001];

base_folder = '~/libmesh-git/libmesh/examples/dphil/coupled_navier_stokes/results/results_for_paper/results_for_paper_linear_iterations/inexact_preconditioner/';

linear_iterations = zeros(555,2);
counter = 0;
...
for prec=5:6
    ...
    if(exist(filename, 'file') == 2)
        ...
        total_linear_iterations = A.data(end,6);
        ...
        linear_iterations(:,counter) = A.data(:,3);
        ...
        % simulation didn't complete for some reason
        if(total_time_steps < expected_time_steps)
            % last residual is greater than previous residual
            if(A.data(end,7) - A.data(end-1,7) > 0)
                % check this isn't the case just cause sim ended at first
                % nonlinear iteration
                if(A.data(end,2) > 1)
