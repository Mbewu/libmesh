...
void ExodusII_IO_Extended::set_output_variables(const std::vector<std::string> & output_variables, bool allow_empty)
{
...
void ExodusII_IO_Extended::copy_nodal_solution(System& system, std::string var_name, unsigned int timestep)
{
  libmesh_deprecated();
...
//write time of simulation to file (say if done discontinuous or something)
...
void ExodusII_IO_Extended::read (const std::string& fname)
{
  ...
  for (int i=0; i<exio_helper->num_elem_blk; i++)
    {
      ...
      const ExodusII_IO_Helper_Extended::Conversion conv = em.assign_conversion(type_str);
...
void ExodusII_IO_Extended::verbose (bool set_verbosity)
{
...
void ExodusII_IO_Extended::use_mesh_dimension_instead_of_spatial_dimension(bool val)
{
...
void ExodusII_IO_Extended::set_coordinate_offset(Point p)
{
  libmesh_deprecated();
...
void ExodusII_IO_Extended::write_element_data (const EquationSystems & es)
{
  ...
  // similar to what is done in ExodusII_IO_Extended::write().  Note that
  // calling ExodusII_IO_Extended::write_timestep() followed by
  // ExodusII_IO_Extended::write_element_data() when the underlying Mesh is a
...
void ExodusII_IO_Extended::write_nodal_data (const std::string& fname,
				    const std::vector<Number>& soln,
				    const std::vector<std::string>& names)
{
  START_LOG("write_nodal_data()", "ExodusII_IO_Extended");
...
void ExodusII_IO_Extended::write_timestep (const std::string& fname,
...
void ExodusII_IO_Extended::write (const std::string& fname)
{
  const MeshBase & mesh = MeshOutput<MeshBase>::mesh();
  ...
  if (_append && _verbose)
    libMesh::out << "Warning: Appending in ExodusII_IO_Extended::write() does not make sense.\n"
...
void ExodusII_IO_Extended::write_nodal_data_discontinuous (const std::string& fname,
                                                  const std::vector<Number>& soln,
                                                  const std::vector<std::string>& names)
{
  ...
  const MeshBase::const_element_iterator end = mesh.active_elements_end();
	...
	//std::cout << "herr" << std::endl;
	// add up the total number of nodes counting shared nodes multiple times
  for ( ; it != end; ++it)
  ...
  STOP_LOG("write_nodal_data_discontinuous()", "ExodusII_IO_Extended");
}



void ExodusII_IO_Extended::write_nodal_data_common(std::string fname,
                                          const std::vector<std::string>& names,
                                          bool continuous)
{
  const MeshBase & mesh = MeshOutput<MeshBase>::mesh();
...
void ExodusII_IO_Extended::write_nodal_data_discontinuous (const std::string&, const std::vector<Number>&, const std::vector<std::string>&)
{
  ...
  void ExodusII_IO_Extended::write_nodal_data_common(std::string,
