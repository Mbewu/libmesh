...
#ifndef LIBMESH_EXODUSII_IO_EXTENDED_H
#define LIBMESH_EXODUSII_IO__EXTENDEDH
...
class ExodusII_IO_Helper_Extended;
class MeshBase;
class System;
...
class ExodusII_IO_Extended : public MeshInput<MeshBase>,
		    ...
		    public ParallelObject
{
 public:
  ...
  void write_discontinuous_exodusII (const std::string& name, const EquationSystems& es, const std::set<std::string>* system_names=NULL);
	 ...
	 */
	void write_time(const int timestep, const Real time);
   ...
   * Write out element solution.
   */
  void write_element_data (const EquationSystems& es, const Real time);
  ...
  /**
   * If true, this flag will cause the ExodusII_IO_Extended object to attempt to
 ...
 private:
  ...
  ExodusII_IO_Helper_Extended *exio_helper;
#endif

  /**
   * Stores the current value of the timestep when calling
   * ExodusII_IO_Extended::write_timestep().
   */
  int _timestep;
...
#endif // LIBMESH_EXODUSII_IO__EXTENDED_H
