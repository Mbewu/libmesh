...
study_name=2d_pipeflow_inexactpressure_pre_stokes_p1p1
...
for i in "${reynolds[@]}"; do
	...
	for j in "${size[@]}"; do
		...
		mpirun -np 1 ./example-opt $fieldsplit_options -output_folder results/$study_name/Re$i/size$j/ -reynolds_number $i -cube_length_N $j -cube_width_N $j
		echo itemSize: $j
	done
done
